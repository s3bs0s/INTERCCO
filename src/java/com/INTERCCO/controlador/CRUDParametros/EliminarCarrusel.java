...
public class EliminarCarrusel extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setCharacterEncoding("UTF-8");
        try {
            String idCarrusel = request.getParameter("idCarr");
            ...
            ps.setInt(1, Integer.parseInt(idCarrusel));
            rs = ps.executeQuery();

            if (rs.next()){
                String nombreImgCarr = cA.DescifrarASCII(rs.getString("nombre_img"));
                
                String SAVE_DIR = "ArchivosSistema" + File.separator + "Carruseles" + File.separator + nomSedeUsuario;
                String appPath = request.getServletContext().getRealPath("");
                String savePath = appPath + SAVE_DIR + File.separator + nombreImgCarr;
                ...
                ps.close();
                rs.close();
                ps = con.prepareStatement("UPDATE carruseles SET existencia=? WHERE idCarruseles=?;");
                ps.setString(1, "N");
                ps.setInt(2, Integer.parseInt(idCarrusel));
                int res = ps.executeUpdate();

                if (res > 0){
                    request.getRequestDispatcher("Parametros?mensaje=YEliminarCarrusel").forward(request, response);
                } else {
                    request.getRequestDispatcher("Parametros?mensaje=Ne&nomMod=El carrusel&accMod=eliminar").forward(request, response);
                    System.out.println("ERROR de ELIMINAR el dato de CARRUSEL.");
                }
            } else {
                request.getRequestDispatcher("Parametros?mensaje=Ne&nomMod=El carrusel&accMod=eliminar").forward(request, response);
                System.out.println("ERROR de ELIMINAR el dato de CARRUSEL.");
